...
export default function USE_fetchVocabsOfAPublicList() {
  const [ARE_vocabsFetching, SET_vocabsFetching] = useState(false);
  const [vocabs_ERROR, SET_error] = useState<string | null>(null);

  const FETCH_vocabsOfPublicList = useCallback(async (list_id: string) => {
  ...
  return { FETCH_vocabsOfPublicList, ARE_vocabsFetching, vocabs_ERROR };
}
