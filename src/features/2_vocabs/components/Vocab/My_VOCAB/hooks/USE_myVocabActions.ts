...
import { PrivateVocabState_PROPS } from "../../../../../../hooks/USE_privateVocabValues";
import USE_zustand from "../../../../../../zustand";
import { useToast } from "react-native-toast-notifications";
import { useSSR, useTranslation } from "react-i18next";
import USE_updateVocabDifficulty from "@/src/features/2_vocabs/hooks/USE_updateVocabDifficulty";
...
export default function USE_myVocabActions({
  ...
  async function UPDATE_privateVocabDifficulty(
    list_ID: string,
    vocab_ID: string,
    newDiffuclty: 1 | 2 | 3
  ) {
    if (allowAction && vocab) {
      const updatedVocab = await UPDATE_vocabDifficulty(
        list_ID,
        vocab_ID,
        newDiffuclty
      );
      if (updatedVocab.success) {
        HIGHLIGHT_vocab(vocab_ID);
      }
    }
  }
