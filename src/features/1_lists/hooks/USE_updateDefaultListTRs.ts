  ...
  const UPDATE_defaultListTRs = async (
    ...
    try {
      ...
      const { error } = await supabase
        .from("lists")
        .update({ default_LANGS: newDefaultTRs })
        .eq("id", targetList_ID);

      // Handle potential errors
      if (error) {
        console.error("🔴 Error updating default_LANGS: 🔴", error);
        SET_updateDefaultTRsError("🔴 Error updating default_LANGS 🔴");
        return { success: false, msg: "🔴 Error updating default_LANGS 🔴" };
      }
      ...
      console.error("🔴 Unexpected error updating default_LANGS: 🔴", error);
