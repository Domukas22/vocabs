...
import FORMAT_listVocabCount from "./FORMAT_listVocabCount"; // Import your function
...
describe("FORMAT_listVocabCount", () => {
  it("should return an empty array if lists is undefined or null", () => {
    expect(FORMAT_listVocabCount(undefined)).toEqual([]);
    expect(FORMAT_listVocabCount(null)).toEqual([]);
  });

  it("should return an empty array if lists is an empty array", () => {
    expect(FORMAT_listVocabCount([])).toEqual([]);
  });
  ...
  it("should assign vocab_COUNT as 0 when vocabs array is not present", () => {
    ...
    const result = FORMAT_listVocabCount(input);
