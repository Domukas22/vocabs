...
import { VALIDATE_fetchVocabArgs } from "./VALIDATE_fetchVocabArgs";

describe("VALIDATE_args", () => {
  test("1. Throws an error when user_id is undefined", () => {
    const args: FETCH_myVocabs_ARG_TYPES = {
      search: "test",
      signal: undefined as unknown as AbortSignal,
      amount: 10,
      user_id: undefined as any,
      list_TYPE: "type1" as list_TYPES,
      excludeIds: new Set(["id1"]),
      fetch_TYPE: "byTargetList" as myVocabFetch_TYPES,
      langFilters: ["en"],
      difficultyFilters: [1],
    ...
    expect(() => VALIDATE_fetchVocabArgs(args)).toThrow(
  ...
  test("2. Throws an error when fetch_TYPE is undefined", () => {
    ...
    expect(() => VALIDATE_fetchVocabArgs(args)).toThrowError(
  ...
  test("12. Passes validation when all arguments are correct", () => {
    ...
    expect(() => VALIDATE_fetchVocabArgs(args)).not.toThrow();
  });
});
